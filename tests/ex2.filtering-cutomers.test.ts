...
import { validateRecordColumns } from 'tests/helpers';
...
class EmployeeDataTest {
  ...
  public async filteredCustomers1() {
    ...
    validateRecordColumns(
      { recordType: 'customer', functionName: 'getAllCustomers({ filter: "fre" })' },
      result[1],
      ALL_CUSTOMERS_REQUIRED_COLS,
      []
    );
  ...
  public async filteredCustomers2() {
    ...
    validateRecordColumns(
      { recordType: 'customer', functionName: 'getAllCustomers({ filter: "mo" })' },
      result[1],
      ALL_CUSTOMERS_REQUIRED_COLS,
      []
    );
