...
class SupplierListViewTest {
  ...
  public async migrationExists() {
    ...
    assert.isAtLeast(migrationsFiles.length, 7, 'There are at least seven things in the ./migrations folder');
    let migrationsSqlsFiles = fs.readdirSync(path.join(__dirname, '..', 'migrations', 'sqls'));
    assert.isAtLeast(migrationsSqlsFiles.length, 12, 'There are at least twelve things in the ./migrations/sqls folder');
  ...
  public async viewExists() {
    ...
    let allViews = (await db.all(sql`select * from sqlite_master where type = 'view';`)).map(i => i.name);
    assert.includeMembers(allViews, ['SupplierList_V'], 'SupplierList_V view is found');
  }
  ...
  public async viewResults() {
    ...
    assert.includeMembers(Object.keys(result), ['Id', 'CompanyName', 'ContactName', 'ProductList'], 'Columns: \'Id\', \'CompanyName\', \'ContactName\', \'ProductList\'');
  }
}
