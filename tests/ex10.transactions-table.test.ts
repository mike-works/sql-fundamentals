...
class TransactionsTableTest {
  ...
  public async transactionsExists() {
    ...
    let allTables = (await db.all(sql`SELECT name FROM sqlite_master WHERE type='table'`)).map(i => i.name);
    assert.includeMembers(allTables, ['Transaction'], 'Transaction table is found');
  }
  ...
  public async insertTransaction() {
    ...
    let transaction = (await db.get(sql`INSERT INTO "Transaction" (Authorization, OrderId) VALUES (?, ?)`, 'lk1hdklh12ld', 10264));
    let afterTransactions = await db.all(sql`SELECT * from "Transaction"`);
    assert.equal(beforeTransactions.length + 1, afterTransactions.length, 'Transaction was inserted');
  }
  ...
  public async invalidOrder() {
    ...
    let errors: string[] = [];
    try {
      let transaction = await db.run(sql`
    INSERT INTO "Transaction" (Authorization, OrderId) VALUES (?, ?)`, 'lk1hdklh12ld', 191927158);
      assert.ok(false, 'Should have encountered a foreign key constraint error');
    } catch (e) {
      errors.push(e.toString());
    }
    assert.isAtLeast(errors.length, 1, 'At least one error (foreign key constraint violation) should have been thrown');
    assert.include(errors[0], 'FOREIGN KEY', 'Error message says something about "FOREIGN KEY');
  }
}
